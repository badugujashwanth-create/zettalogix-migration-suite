...
| Area | v0.3 verified evidence | Remaining gap | Honest status |
|---|---|---|---|
| Primary workflow | search, blueprint, create/start/pause, history, reset, export | no owned worker or production content move | strong synthetic prototype |
| Readiness | five checks from current connections, mappings, failures, and events | no discovery inventory or worker preflight contract | evidence-derived client assessment |
| Identity | fail-closed Supabase client boundary | no tenant-specific OAuth validation | configurable, unverified live mode |
| Providers | fields and guidance for supported connection types | no authorized tenant integration test | UI contract only |
